...
do
	...
	if [ "$inputvar" == '?' ]; then
		...
		if [ "${#item_dir_array[@]}" == 1 ]; then
			...
			cmd=$( func_dialog_menu '請從裡面挑一項你所要的(本層的資料夾)' 100 "$dialogitems" $tmpfile )
		...
		if [ "${#item_parent_file_array[@]}" -eq 1 ]; then
			...
			dialogitems=''
			for echothem in ${item_parent_file_array[@]}
			do
				dialogitems=" $dialogitems $echothem '' "
			done
		...
		if [ "${#item_parent_dir_array[@]}" == 1 ]; then
			...
			cmd=$( func_dialog_menu '請從裡面挑一項你所要的(上層的資料夾)' 100 "$dialogitems" $tmpfile )
